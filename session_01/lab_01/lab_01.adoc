...
. Go to the https://drive.google.com/open?id=1imK3lLq8A4cc440RVM1-9zkDu4ClobLJpnHnsBpqmpk[user info spreadsheet] and assign yourself a Pivotal Cloud Foundry user for the workshop.		

. From a command prompt, set the API target for the CLI:
+
----
$ cf api https://api.run.haas-74.pez.pivotal.io --skip-ssl-validation
----
...
Here the droplet is being uploaded to PWS's internal blobstore so that it can be easily copied to one or more _http://docs.cloudfoundry.org/concepts/how-applications-are-staged.html[Diego Cells]_ for execution.
