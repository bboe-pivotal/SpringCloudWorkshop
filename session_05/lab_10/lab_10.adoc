...
. Make sure you're logged in to your GitHub account, and create a new repository called "config-repo":
+
...
+
----
$ git remote add origin https://github.com/<your-account>/config-repo.git
...
+
----
$cf create-service p-config-server standard new-config-server -c '{"git": { "uri": "<Your-Git-Repository-URL>" } }'
